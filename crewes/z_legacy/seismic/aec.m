...
%test for row or column vector
[nr,nc]=size(trin);
if((nr-1)*(nc-1))
	error('aec cannot handle multiple traces');
end
if(nr==1)
	trin=trin.';
end
...
% stabilize the envelope at the ends
 ...
 envsm=[envsm(ntrip)*ones(ntrip,1); envsm(ntrip+1:length(envsm))];
 envsm=[envsm(1:length(envsm)-ntrip); envsm(length(envsm)-ntrip)*ones(ntrip,1)];
% correct the trace
 trout=trin./envsm;
if(nr==1);
	trout=trout.';
